    ...
    match intrinsic {
        ...
        | "llvm.x86.avx2.gather.q.pd.256" => {
            ...
            for lane_idx in 0..std::cmp::min(src_lane_count, index_lane_count) {
                ...
                let mask_lane = match mask_lane_clif_ty {
                    types::I32 | types::F32 => {
                        fx.bcx.ins().band_imm_s(mask_lane, 0x8000_0000u64 as i64)
                    }
                    types::I64 | types::F64 => {
                        fx.bcx.ins().band_imm_s(mask_lane, 0x8000_0000_0000_0000u64 as i64)
                    }
                    _ => unreachable!(),
                };
        ...
        "llvm.x86.ssse3.pshuf.b.128" | "llvm.x86.avx2.pshuf.b" => {
            ...
            for i in 0..16 {
                ...
                let is_zero = fx.bcx.ins().band_imm_s(b_lane, 0x80);
                let a_idx = fx.bcx.ins().band_imm_s(b_lane, 0xf);
            ...
            if intrinsic == "llvm.x86.avx2.pshuf.b" {
                for i in 16..32 {
                    let b_lane = b.value_lane(fx, i).load_scalar(fx);
                    let is_zero = fx.bcx.ins().band_imm_s(b_lane, 0x80);
                    let b_lane_masked = fx.bcx.ins().band_imm_s(b_lane, 0xf);
                    let a_idx = fx.bcx.ins().iadd_imm_s(b_lane_masked, 16);
                    let a_idx = fx.bcx.ins().uextend(fx.pointer_type, a_idx);
                    let a_lane = a.value_lane_dyn(fx, a_idx).load_scalar(fx);
                    let res = fx.bcx.ins().select(is_zero, zero, a_lane);
                    ret.place_lane(fx, i).to_ptr().store(fx, res, MemFlags::trusted());
        ...
        | "llvm.x86.avx.vperm2f128.pd.256" => {
            ...
            fn select4(
                ...
                let a_or_b = fx.bcx.ins().band_imm_s(control, 0b0010);
                let high_or_low = fx.bcx.ins().band_imm_s(control, 0b0001);
                let is_zero = fx.bcx.ins().band_imm_s(control, 0b1000);
            ...
            let control1 = fx.bcx.ins().ushr_imm_s(imm8, 4);
        ...
        "llvm.x86.sse2.pavg.b" | "llvm.x86.sse2.pavg.w" => {
            ...
            simd_pair_for_each_lane(
                ...
                &|fx, _lane_ty, _res_lane_ty, a_lane, b_lane| {
                    ...
                    let num_plus_one = fx.bcx.ins().iadd_imm_s(sum, 1);
                    let res = fx.bcx.ins().ushr_imm_s(num_plus_one, 1);
        ...
        "llvm.x86.ssse3.pmul.hr.sw.128" => {
            ...
            for out_lane_idx in 0..lane_count {
                ...
                let shifted = fx.bcx.ins().ushr_imm_s(mul, 14);
                let incremented = fx.bcx.ins().iadd_imm_s(shifted, 1);
                let shifted_again = fx.bcx.ins().ushr_imm_s(incremented, 1);
        ...
        "llvm.x86.avx.ptestz.256" => {
            ...
            let res = fx.bcx.ins().icmp_imm_s(IntCC::Equal, all_zero, 0);
