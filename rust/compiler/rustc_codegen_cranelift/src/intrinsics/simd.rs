    ...
    match intrinsic {
        ...
        sym::simd_select => {
            ...
            for lane in 0..lane_count {
                ...
                let m_lane = fx.bcx.ins().icmp_imm_s(IntCC::Equal, m_lane, 0);
                let res_lane =
        ...
        sym::simd_select_bitmask => {
            ...
            for lane in 0..lane_count {
                ...
                let m_lane = fx.bcx.ins().ushr_imm_s(m, mask_lane.cast_signed());
                let m_lane = fx.bcx.ins().band_imm_s(m_lane, 1);
                let a_lane = a.value_lane(fx, lane).load_scalar(fx);
                let b_lane = b.value_lane(fx, lane).load_scalar(fx);
        ...
        sym::simd_bitmask => {
            ...
            for lane in lanes {
                ...
                let a_lane_sign =
                    fx.bcx.ins().ushr_imm_s(a_lane, i64::from(lane_clif_ty.bits() - 1));

                // shift sign bit into result
                let a_lane_sign = clif_intcast(fx, a_lane_sign, res_type, false);
                res = fx.bcx.ins().ishl_imm_s(res, 1);
        ...
        sym::simd_arith_offset => {
            ...
            for lane_idx in 0..lane_count {
                ...
                let ptr_diff = if pointee_size != 1 {
                    fx.bcx.ins().imul_imm_s(offset_lane, pointee_size as i64)
                } else {
                    offset_lane
                };
