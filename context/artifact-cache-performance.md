...
This investigation uses uv as a pinned representative workload, not uv CI as
the test harness. The branch is based on SRS `4a8cc277e`; the measurements below
were collected across successive implementation commits and are historical
unless a table explicitly identifies final HEAD. Most ran locally on an Apple
silicon macOS runner with:
...
The thin layer is 0.53s slower at the local median and its archive contains 167
MiB less. If the portable artifact cache is already resident and request
latency is ignored, it would win once effective archive transfer bandwidth is
below about 315 MiB/s. Downloading both layers was not measured, so this is a
local hypothesis rather than a transfer result. Both layers remain useful
integration controls: full is the fastest local fallback, while thin may avoid
paying remote-cache bandwidth for bytes already present locally. The retained target still contains uncached library
...
### Finalized custom-build library actions

The next runtime expansion removed the blanket rejection of every library in a
package with a custom build script. It does not cache the build script or admit
the package to graph preflight. Cargo runs the script first, applies its final
cfg, environment, native, search-path, and loader directives, and only then
describes the ordinary library rustc action. The final command, explicit
environment, complete `OUT_DIR` trees, direct externs, loader roots, and
rustc-discovered inputs remain content-bound. Native-link directives and other
unmodeled final actions remain ineligible, while a generated dep-info input
under a build/output directory still prevents publication.

The same pinned generation workload and already-warm cache produced these
successful empty-target samples:

| Runtime policy | Wall samples | Eligible / ineligible | Hits / misses | Custom-build rejection | rustc / build scripts |
| --- | ---: | ---: | ---: | ---: | ---: |
| Package blanket rejection | 50.39s, 53.55s | 385 / 177 | 305 / 80 | 55 | 257 / 55 |
| Finalized-action admission | 47.85s, 48.40s | 430 / 132 | 337 / 93 | 0 | 225 / 55 |

The new policy admitted 45 additional actions. On each fully warmed target, 32
restored, five remained target-specific misses that were republished, and eight
were safely left unpublished because their generated dep-info inputs were not
portable. Restored output increased from 610 files / 496.1 MB to 674 files /
625.7 MB, while preflight remained at 188 finalized units and every one of the
55 build scripts still executed. The midpoint of the two directional wall-time
...
not included in the wall-time table. Focused Cargo regressions cover stable
cross-target restoration after the script runs, `OUT_DIR` and emitted-env
changes, generated-source publication refusal, native-link rejection, final
loader input mutation, unsafe final loader environment, and macOS
`HOME`-controlled dyld fallback inputs.
...
At the point represented by this historical trace, the cache still rejected
proc macros, build-script executables, test binaries, test-mode Check actions,
and Clippy wrapper actions. A finalized ordinary
library action from a package with a custom build script can now restore after
the script runs, but generated dep-info inputs are not yet publishable and
target-specific generated bytes still produce safe misses. A consumer's action
key hashes raw dependency artifacts. When one of those remaining uncached
dependencies embeds an absolute `OUT_DIR` or target path, its bytes differ on
the next runner and every downstream consumer misses even if its own source and
...
The current protocol covers pure Build-library units. Metadata-only Check
library actions, including test-mode library checks, are cacheable but still
use per-unit dirty replay until their
...
Packages with build scripts remain outside graph-wide preflight, but their
finalized ordinary library actions can now use the runtime artifact cache after
the script runs. Skipping the script or making the whole package Cargo-fresh
...
the correct binary, but reported generic missing patch metadata and performed a
full relink. Four controlled edits repeated that fallback. Inspection showed
that the retained `.rlib` snapshot was valid and its 20-byte `__text` section
matched the replacement output. The actual blocker is SLD's lazy
`FilePatchState` derivation: it currently admits Mach-O `__data`, `__const`, and
`__cstring`, but refuses `__text`, so no patch state is produced for this input.
These are tiny directional correctness samples, not representative timings,
but the linker log and retained input establish a specific SLD eligibility gap,
not cache corruption or missing producer ownership.
...
win has a narrow next experiment: admit canonical Mach-O `__TEXT,__text`
`S_REGULAR` sections only when metadata creation and materialization both prove
an unchanged exact size, zero relocations, and no padding, while retaining the
existing archive, symbol, unwind, layout, and signature gates. A sibling of the
existing signed private-rlib `__data` fixture should cover `__text`. This is an
SLD patch-state eligibility problem, not a reason to restore a larger archive
or weaken artifact-cache identity.
...
3. Add opt-in structured counters and cumulative phase timing so hit rates,
   ...
   parsing debug logs.
4. Keep metadata-only rustc library Check actions, including test-mode library
   variants, copy-materialized and covered by cache-immutability, source-mutation,
   downstream-consumption, and next-run freshness tests. Keep linked test
   binaries ineligible.
...
- action-input hash calls, files/logical bytes, failures, and time, plus
  process-local witness checks, fast paths, fallbacks, and time;
...
remains a useful future schema addition if the v1 measurements show that the
current aggregates cannot explain replay cost.
...
performance gates remain open until the base/branch warm-cache matrix is
complete.
...
proc-macro, and final-link state as a fallback, but that does not close the
portable cache-scope goal. uv PR 19754 should combine only measured layers and
compare equivalent runner classes after the local matrix is complete. The
aggregate CI acceptance target remains a consumer validation step rather than
the diagnostic or product-development harness for this investigation.
